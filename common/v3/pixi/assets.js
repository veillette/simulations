...
Assets.Texture = function(filename) {
    ...
    if (useAssetsApi) {
        // Pixi v7+: Assets.get() uses the same key resolution as Assets.load(),
        // so it finds the texture regardless of whether the URL was resolved to
        // an absolute path internally.  Only call this after assets are loaded.
        return PIXI.Assets.get(pathPrefixedFilename) || PIXI.Assets.get(filename) || null;
    }

    // Pixi v5 and below: walk loader.resources
