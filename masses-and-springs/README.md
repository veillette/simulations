...
3. From the root `masses-and-springs` directory, run `npm install` in the command line to install test and build dependencies.
...
From the root `masses-and-springs` directory, run `grunt dist`.

The `dist` directory containing the built site will be added to the root `masses-and-springs` directory.

### Testing

To run command-line tests, run `grunt test` from the root `masses-and-springs` directory.
...
3. then go to [http://localhost:8080/masses-and-springs/test/](http://localhost:8080/masses-and-springs/test/). 
...
From the root `masses-and-springs` directory, run `npm run-script upgrade`, which executes the following commands:
