...
3. From the root `greenhouse-effect` directory, run `npm install` in the command line to install test and build dependencies.
...
From the root `greenhouse-effect` directory, run `grunt dist`.

The `dist` directory containing the built site will be added to the root `greenhouse-effect` directory.

### Testing

To run command-line tests, run `grunt test` from the root `greenhouse-effect` directory.
...
3. then go to [http://localhost:8080/greenhouse-effect/test/](http://localhost:8080/greenhouse-effect/test/). 
...
From the root `greenhouse-effect` directory, run `npm run-script upgrade`, which executes the following commands:
