...
3. From the root `ladybug-motion` directory, run `npm install` in the command line to install test and build dependencies.
...
From the root `ladybug-motion` directory, run `grunt dist`.

The `dist` directory containing the built site will be added to the root `ladybug-motion` directory.

### Testing

To run command-line tests, run `grunt test` from the root `ladybug-motion` directory.
...
3. then go to [http://localhost:8080/ladybug-motion/test/](http://localhost:8080/ladybug-motion/test/). 
...
From the root `ladybug-motion` directory, run `npm run-script upgrade`, which executes the following commands:
