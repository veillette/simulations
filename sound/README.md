...
3. From the root `sound` directory, run `npm install` in the command line to install test and build dependencies.
...
From the root `sound` directory, run `grunt dist`.

The `dist` directory containing the built site will be added to the root `sound` directory.

### Testing

To run command-line tests, run `grunt test` from the root `sound` directory.
...
3. then go to [http://localhost:8080/sound/test/](http://localhost:8080/sound/test/). 
...
From the root `sound` directory, run `npm run-script upgrade`, which executes the following commands:
