...
3. From the root `arrow-test` directory, run `npm install` in the command line to install test and build dependencies.
...
From the root `arrow-test` directory, run `grunt dist`.

The `dist` directory containing the built site will be added to the root `arrow-test` directory.

### Testing

To run command-line tests, run `grunt test` from the root `arrow-test` directory.
...
3. then go to [http://localhost:8080/arrow-test/test/](http://localhost:8080/arrow-test/test/). 
...
From the root `arrow-test` directory, run `npm run-script upgrade`, which executes the following commands:
