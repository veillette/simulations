...
3. From the root `states-of-matter` directory, run `npm install` in the command line to install test and build dependencies.
...
From the root `states-of-matter` directory, run `grunt dist`.

The `dist` directory containing the built site will be added to the root `states-of-matter` directory.

### Testing

To run command-line tests, run `grunt test` from the root `states-of-matter` directory.
...
3. then go to [http://localhost:8080/states-of-matter/test/](http://localhost:8080/states-of-matter/test/). 
...
From the root `states-of-matter` directory, run `npm run-script upgrade`, which executes the following commands:
